...
- **查询编排层（QueryPipeline）**：`pipelines/query_pipeline.py`
...
- **主模型**：回答用户问题、用于图查询引擎。
- **抽取模型**：构建知识图谱时使用，通常是更小的模型。
- **Embedding 模型**：生成向量存储到 pgvector。

所有模型的请求地址由 `OLLAMA_BASE_URL` 和对应 model 名配置；超时时间通过 `REQUEST_TIMEOUT` / `EXTRACTION_TIMEOUT` 控制。
